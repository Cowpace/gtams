...
<html>
	...
	<body><center>
			<?php
				$result = $mysqli->query('SELECT * FROM nomination WHERE nomination_id = 1')->fetch_object();
				$result2 = $mysqli->query('SELECT * FROM ListGradCourse WHERE nomination_id = 1');
				$result3 = $mysqli->query('SELECT * FROM ListPublication WHERE nomination_id = 1');
			?>
			<br>
			<table>
			<caption>Fedora's Info</caption>
